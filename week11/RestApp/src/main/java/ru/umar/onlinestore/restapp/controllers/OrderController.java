...
public class OrderController {
    ...
    public ResponseEntity<HttpStatus> create(@RequestParam int buyer_id,
        ...
        orderService.save(buyer_id, t_shirt_id, order_date);
        // Отправляем http ответ с пустым телом и со статусом 200
        return ResponseEntity.ok(HttpStatus.OK);
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<String> update(@PathVariable("id") int id, @RequestParam(value = "buyer_id") int buyer_id,
                                             @RequestParam(value = "t_shirt_id") int t_shirt_id,
                                             @RequestParam(value = "order_date") String order_date){


        return orderService.update(id, buyer_id, t_shirt_id, order_date);
    }

    @DeleteMapping("/delete/{id}")
    public Order delete(@PathVariable("id") int id){
